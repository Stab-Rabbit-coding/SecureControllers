...
- **Verified the boards load** in `pcbnew` (Wash 43, TACCO 42, Emma 69,
...
1. **Tamper mesh (`TMESH_P`/`TMESH_N`).** Drawn today as a raw
   ...
   **FlightEngineer and Emma have no local TPM**, so their tamper signal must be
   carried over the inter-board link (Emma → TACCO, FlightEngineer → Wash).
...
- **Recommended workflow:** finish routing in the **KiCad GUI** (where
  ...
  (per-domain tamper-mesh zones, cosmetic/footprint DRC, the Emma/FlightEngineer
